...
unionsMap f bag = unions . List.map (timesF' f) . toOccurList $ bag
--    where timesF (ms,1) = f ms
--          timesF (ms,n) = MS . Map.map (*n) . unMS $ f ms

-- unionsMap' :: (Ord b) => (a -> MultiSet b) -> MultiSet a -> MultiSet b
unionsMap' f bag = List.map (timesF' f) . toOccurList $ bag

timesF' :: (a -> MultiSet b) -> (a, Occur) -> MultiSet b
timesF' f (ele, occ)
     | (==) occ 1 = f ele
     | otherwise  = MS . Map.map (*occ) . unMS $ f ele
